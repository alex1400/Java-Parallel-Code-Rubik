...
public class Rubiks {
    ...
    public static ArrayList<Cube> allJobs; //An array to save all the sub jobs or leaves.
    /**
     ...
     */
    // A method to create the children cubes to the second level.
    private static int CreateJobList(Cube cube, CubeCache cache) {
        if (cube.isSolved()) {
            return 1;
        }
        ...
        for (Cube child : children) {
            // recursion step
            int childSolutions = CreateJobList(child, cache);
            if (childSolutions > 0) {
    ...
    // A method to create the children cubes to the secons level.
    private static int CreateJobs(Cube cube) {
        ...
        while (result == 0 && bound!=2) {
            bound++;
            cube.setBound(bound);
            result = CreateJobList(cube, cache);
        }
        return result;
    }
    ...
    public static void main(String[] arguments) throws Exception {
        ...
        int result=CreateJobs(cube);    
        Communication communication = new Communication(allJobs, cube);
        if(communication.myIbis.identifier().equals(communication.server)){
            // print cube info by master instance
        ...
        if(result!=0 && communication.myIbis.identifier().equals(communication.server)){
            solve(cube);
        } if (result==0){
            communication.run();
        }
        ...
        //Print the execution time by master node
        if(communication.myIbis.identifier().equals(communication.server))
